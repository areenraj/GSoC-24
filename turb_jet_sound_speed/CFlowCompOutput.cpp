...
void CFlowCompOutput::SetHistoryOutputFields(CConfig *config){
  ...
  /// DESCRIPTION: Speed of Sound Averaged over the Mesh
...
void CFlowCompOutput::LoadHistoryData(CConfig *config, CGeometry *geometry, CSolver **solver)  {
  ...
  vector<su2double> Surface_SoundSpeed_Local              (nMarker_Analyze,0.0);

  vector<su2double> Surface_SoundSpeed_Total              (nMarker_Analyze,0.0);
    ...
    if (config->GetMarker_All_Analyze(iMarker) == YES)  {
      ...
      for (iMarker_Analyze= 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
        ...
        if (config->GetMarker_All_TagBound(iMarker) == config->GetMarker_Analyze_TagBound(iMarker_Analyze)) {
          Surface_SoundSpeed_Local[iMarker_Analyze] += Surface_SoundSpeed[iMarker];
  ...
  Allreduce(Surface_SoundSpeed_Local, Surface_SoundSpeed_Total);
  ...
  for (iMarker_Analyze = 0; iMarker_Analyze < nMarker_Analyze; iMarker_Analyze++) {
    ...
    su2double SoundSpeed = Surface_SoundSpeed_Total[iMarker_Analyze];
    Tot_Surface_SoundSpeed += SoundSpeed;
  }
